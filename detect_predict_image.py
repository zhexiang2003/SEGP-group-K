...
def detection_prediction(image_path, 
    ...
    print('image loaded for seed detection.')
    ...
    if torch.cuda.is_available():
        ...
        model_state_dict = torch.load(model_path, map_location=torch.device('cpu'))  ### model could not be loaded, not sure what the reason is atm!!!###
        
    print('model {} is loaded.'.format(model_path)) 
    
    model.load_state_dict(model_state_dict)
    # move model to the right device
    model.to(device)
    
    print('A trained Faster R-CNN model is loaded.')
    ...
    print('Faster R-CNN detection is done. Now running Non-Maximal suppression.')
    ...
    print('Non-maximal suppression is done.')
    ...
    return rows, image_with_bboxes


# define run_seed_detection which will be used in the main.py
def run_seed_detection(input_filename):
    input_filename = r'/home/segp/PI_SEGP_BACKUP/resources/temp/current_input/' + input_filename
    trained_model = 'fasterrcnn_resnet50_fpn_state_dict.pth'
    model_path = os.path.join('/home/segp/PI_SEGP_BACKUP/models_old/', trained_model)
    outpath = r'/home/segp/PI_SEGP_BACKUP/out'
    ...
    rows, image_with_bbox = detection_prediction(input_filename,
                                                model_path,
                                                outpath,
                                                device=device,
                                                confidence_threshold=confidence_threshold)
    
    print('Faster R-CNN detection performed.')

    laser_matrix = [[0,0],[0,0]]

    print('The total number of seeds detected is {}'.format(len(rows)))
    ...
    for i in range(len(rows)):
        if rows[i][5] == 'BAD':
            ...
            label = 1
        if i == 1:
            laser_matrix[0][0] = rows[i][5]
        elif i == 2:
            laser_matrix[0][1] = rows[i][5]
        elif i == 3:
            laser_matrix[1][0] = rows[i][5]
        elif i == 4:
            laser_matrix[1][1] = rows[i][5]
        if rows[i][5] == 1:
            good_seeds = good_seeds + 1
        elif rows[i][5] == 0:
            bad_seeds = bad_seeds + 1
    print('laser matrix is obtained.')

    tray_id = os.path.basename(input_filename)
    tray_id = os.path.splitext(tray_id)[0]
    print('Tray {} is processed'.format(tray_id))
    ...
    imagename = input_filename.rsplit('/', 2)[2][:-4]
    print('Image name to be saved is {}'.format(imagename))
    cv2.imwrite(os.path.join(folder_name_bound, imagename + '.JPG'), cv2.cvtColor(image_with_bbox, cv2.COLOR_RGB2BGR))
